  ...
  try {
    ...
    if (configResponse.ok) {
      ...
      if (config.assessmentUrl) {
        assessmentUrl = config.assessmentUrl;
        console.log('Loaded assessment URL from server:', assessmentUrl);
      }
    }
  } catch (error) {
    console.warn('Could not fetch config from server, using default assessment URL:', error);
  }
